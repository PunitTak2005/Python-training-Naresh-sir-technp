...
def save_act():
    ...
    if savecon.upper() == "YES":
        with open("punit.txt", "w") as filehandle:
...
def load_info():
    messagebox.showinfo(
        "Info", "Created by PUNIT TAK")


def load_act():
    loadcon = messagebox.askquestion(
        'Load Confirmation', 'Load your previous progress?')
    if loadcon.upper() == "YES":
        tasks.clear()
        with open('punit.txt', 'r') as filereader:
...
root.title("TECHNO NJR INSTITUTE OF TECHNOLOGY, UDAIPUR")
...
label_title = tkinter.Label(root, text="Todo List", bg="pink", fg="blue")
label_title.grid(row=0, column=0)

label_dsply = tkinter.Label(root, text="Tasks", bg="pink", fg="blue")
...
sort_dsc = tkinter.Button(root, text="Sort (DSC)", bg="White", fg="green", width=20, height=2, command=sort_dsc)
...
number_task = tkinter.Button(
    root, text="Number of Tasks", bg="white", fg="green", width=20, height=2, command=number_task)
...
load_button = tkinter.Button(
    root, text="Load Last TodoList", bg="white", fg="green", width=20, height=2, command=load_act)
